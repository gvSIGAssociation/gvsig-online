...
function configure() {
	...
	if [ -z $GVSIGOL_HOST ]; then
		...
		export BASE_URL="$HTTP_PROTOCOL:\/\/$GVSIGOL_HOST"
	...
	if [ -z $GVSIGOL_HOME ]; then
		...
		export GVSIGOL_HOME="/opt/gvsigonline"	
	fi
	...
	if [ -z $GVSIGOL_PASSWD ]; then
		...
		export GVSIGOL_PASSWD="##GVSIGOL_PASSWD##"
	else
	...
	if [ -z $FRONTEND_URL ]; then
		...
		export FRONTEND_URL="externo.gva.es"
	fi
	...
	if [ -z $GEOSERVER_HOST ]; then
		...
		export GEOSERVER_HOST=$GVSIGOL_HOST
	fi
	...
	if [ -z $GEOSERVER_REST_URL ]; then
		...
		export GEOSERVER_REST_URL="$HTTP_PROTOCOL://$GEOSERVER_HOST/geoserver/rest"
	fi
	export GEOSERVER_BASE_URL="$HTTP_PROTOCOL://$GVSIGOL_HOST/geoserver"
	export GEOWEBCACHE_REST_URL="$HTTP_PROTOCOL://$GVSIGOL_HOST/geoserver/gwc/rest"
	...
	if [ -z $GEOSERVER_PASSWD ]; then
		...
		export GEOSERVER_PASSWD="##GEOSERVER_PASSWD##"
	fi
	...
	if [ -z $LDAP_ROOT_DN ]; then
		...
		export LDAP_ROOT_DN="dc=local,dc=gvsigonline,dc=com"
	fi
	...
	if [ -z $LDAP_BIND_USER ]; then
		...
		export LDAP_BIND_USER="cn=admin,$LDAP_ROOT_DN"
	fi
	...
	if [ -z $LDAP_BIND_PASSWD ]; then
		...
		export LDAP_BIND_PASSWD="##LDAP_BIND_PASSWD##"
	fi
	...
	if [ -z $CRS_FROM_SETTINGS ]; then
		...
		export CRS_FROM_SETTINGS="False"
	fi
	...
	if [ -z $AUTH_WITH_REMOTE_USER ]; then
		...
		export AUTH_WITH_REMOTE_USER="False"
	else
		if [ "$AUTH_WITH_REMOTE_USER" = "true" ]; then
			export AUTH_WITH_REMOTE_USER=True
		else
			export AUTH_WITH_REMOTE_USER=False
		fi		
	fi
	...
	if [ -z $FILEMANAGER_DIR ]; then
		...
		export FILEMANAGER_DIR="/var/www/media/data"
	fi												
	...
	if [ -z $GDALTOOLS_BASEPATH ]; then
		...
		export GDALTOOLS_BASEPATH="/usr/bin"
	fi														
	...
	if [ -z $OGR2OGR_PATH ]; then
		...
		export OGR2OGR_PATH="$GDALTOOLS_BASEPATH/ogr2ogr"
	fi														
	...
	if [ -z $EMAIL_BACKEND_ACTIVE ]; then
		export EMAIL_BACKEND_ACTIVE="False"
	else
		if [ "$EMAIL_BACKEND_ACTIVE" = "true" ]; then
			export EMAIL_BACKEND_ACTIVE=True
		else
			export EMAIL_BACKEND_ACTIVE=False
		fi		
	fi
	...
	#TODO: hay que llevarlo a la app 
	if [ "$LANGUAGES" = "" ]; then
		export LANGUAGES="('en', _('English')), ('es', _('Spanish'))"
	fi
	...
	if [ -z $GVSIGOL_ENABLE_ENUMERATIONS ]; then
		...
		export GVSIGOL_ENABLE_ENUMERATIONS="True"
	fi
