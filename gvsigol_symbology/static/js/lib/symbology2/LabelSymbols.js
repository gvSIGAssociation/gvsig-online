...
var LabelSymbols = function(workspace, layer, style, type, buttoncomp, panelcomp, fields, sldFilterValues) {	
...
LabelSymbols.prototype.initializeButtonComponent = function(component) {
var container = $("<li>", {class:"collection-item avatar"});
	
	var icon = $("<i>", {text: "folder", class:"material-icons circle"});
	var title = $("<span>", {text: gettext("Etiquetado"), class:"title"});
    var desc = $("<p>", {html: gettext("Etiquetado descripcion")});
 ...
 */
LabelSymbols.prototype.initializePanelComponent = function(component, fields) {
	var panel = $("<div>", {id: this.getID() + "-panel"});
	...
	var title = $("<span>", {class:"legend-title", text: gettext("Etiquetado")});
...
LabelSymbols.prototype.getID = function() {
	return this.legendId;
};


LabelSymbols.prototype.updateFilterField = function(rule, filterText, minscale, maxscale) {
	...
	$(".filter-text.label-rule-"+rule).text(filterText);
 ...
 * Load Data
 */
LabelSymbols.prototype.loadData = function(data) {
	var previews = []
	...
	for(var i=0;i<data.length; i++){
		...
		for(var j=0; j<data[i].symbols.length; j++){
			...
			var fieldInput =  $("<span>", {
				class: "legend-row validate filter-text label-rule-"+i, 
				text: filterText, 
				rule: i,
				symbol: j});
	...
	$(".load-symbol-form-button").unbind("click").click(function(){
		...
		if(SLDDefaultValues[that.type].Label && 
			SLDDefaultValues[that.type].Label.definition){
				SLDDefaultValues[that.type].Label.definition.values = JSON.stringify(fds);
				for(keys in fds){
					SLDDefaultValues[that.type].Label.definition.defaultValue = keys;
					break;
				}
		}
		
		var sld = that.currentData[rule].symbols[symbol].fields.sld_code;
		var result = sld.match(/<PropertyName>(.*?)<\/PropertyName>/g);
		if(result != null && result != ""){
			var fieldLabel = result[0].replace(/<\/?PropertyName>/g,'');
			SLDDefaultValues[that.type].Label.definition.defaultValue = fieldLabel;
		}
...
LabelSymbols.prototype.loadPreviewMap = function(workspace, layer, username, password) {	
	this.previewMap.loadMap(workspace, layer, username, password);
};

LabelSymbols.prototype.createNewData = function() {
	var symbologyUtils = new SymbologyUtils();
	
	// Creamos la regla con un símbolo estándar 
	var fieldName = gettext("new-label");
	var fieldFilter = "";
...
LabelSymbols.prototype.loadPreviews = function(previews) {
 ...
 * Get Data
 */
LabelSymbols.prototype.getData = function() {
