...
if [ -z "$CATALOG_AUTO_CREATE_METADATA" ]; then
        echo "WARNING: CATALOG_AUTO_CREATE_METADATA is not defined, using 'True'"
        CATALOG_AUTO_CREATE_METADATA="True"
fi
if [ -z "$CATALOG_METADATA_STANDARD" ]; then
        echo "WARNING: CATALOG_METADATA_STANDARD is not defined, using '' (ISO 19139:2007)."
        CATALOG_METADATA_STANDARD=""
fi
grep -rl "##GEONETWORK_USE_KEEPALIVE##"  | xargs sed -i "s/##GEONETWORK_USE_KEEPALIVE##/$GEONETWORK_USE_KEEPALIVE/g"
grep -rl "##CATALOG_AUTO_CREATE_METADATA##"  | xargs sed -i "s/##CATALOG_AUTO_CREATE_METADATA##/$CATALOG_AUTO_CREATE_METADATA/g"
grep -rl "##CATALOG_METADATA_STANDARD##"  | xargs sed -i "s/##CATALOG_METADATA_STANDARD##/$CATALOG_METADATA_STANDARD/g"
...
echo "CATALOG_AUTO_CREATE_METADATA" $CATALOG_AUTO_CREATE_METADATA
echo "CATALOG_METADATA_STANDARD" $CATALOG_METADATA_STANDARD
