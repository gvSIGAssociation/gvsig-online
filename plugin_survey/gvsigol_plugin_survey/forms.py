...
class SurveySectionForm(forms.ModelForm):
    ...
    srs = forms.ChoiceField(label=_('SRS'), required=False, choices=supported_srs_and_blank, widget=forms.Select(attrs={'class' : 'form-control js-example-basic-single'}))
    definition = forms.CharField(label=_('Definition'), required=False, widget=forms.Textarea(attrs={'class': 'form-control', 'tabindex': '4'}))


class UploadFileForm(forms.Form):
    name = forms.ModelChoiceField(label=_('Name'), required=True, queryset=Survey.objects.all().order_by('name'), widget=forms.Select(attrs={'class':'form-control js-example-basic-single'}))
    fileupload = forms.FileField(label=_('File'), required=True, widget=forms.FileInput(attrs={'accept' : '.sqlite,.gpap'}))