...
var Charts = function(featureType, layerName, utils, rule_opts) {
	this.selected = null;
	...
	if (rule_opts != null) {
		if (rule_opts.symbolizers != "") {
			...
			$('#rules').append(rule.getTableUI(false, 'charts'));
			rule.registerEvents('charts');
			rule.preview();
...
Charts.prototype.showLabel = function() {
	if (this.label) {
...
Charts.prototype.loadLabel = function(options) {
	if (this.label) {
		this.label = null;
...
Charts.prototype.updateLabelForm = function() {
	$('#tab-menu').empty();
	$('#tab-content').empty();	
...
Charts.prototype.getRules = function() {
	return this.rules;
};

Charts.prototype.getRuleById = function(id) {
...
Charts.prototype.addRule = function(rule) {
	return this.rules.push(rule);
};

Charts.prototype.deleteRule = function(id) {
	for (var i=0; i < this.rules.length; i++) {
		if (this.rules[i].id == id) {
...
Charts.prototype.load = function(selectedField, values) {
	$('#rules').empty();
...
Charts.prototype.loadRules = function(rules) {
	$('#rules').empty();
	this.rules.splice(0, this.rules.length);
...
Charts.prototype.refreshMap = function() {
	this.utils.updateMap(this, this.layerName);
};

Charts.prototype.save = function(layerId) {
	$("body").overlay();
	...
	$.ajax({
		type: "POST",
		async: false,
		url: "/gvsigonline/symbology/charts_add/" + layerId + "/",
...
Charts.prototype.update = function(layerId, styleId) {
	$("body").overlay();
	style = this.getStyleDef();
	...
	$.ajax({
		type: "POST",
		async: false,
		url: "/gvsigonline/symbology/charts_update/" + layerId + "/" + styleId + "/",
		beforeSend:function(xhr){
			xhr.setRequestHeader('X-CSRFToken', Cookies.get('csrftoken'));
		},
		data: {
...
Charts.prototype.getStyleDef = function(layerId) {
	var minscale = $('#symbol-minscale').val();
...
Charts.prototype.updatePreview = function(layerId) {
	var self = this;
	//$("body").overlay();
	style = this.getStyleDef();
