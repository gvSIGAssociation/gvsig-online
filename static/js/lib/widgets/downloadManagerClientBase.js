...
DownloadManagerUI.prototype.layerDirectDownloads = function(layer) {
	if (!layer.baselayer) {
		if (layer.wfs_url) {
			...
			var layerResourcesLink = '/gvsigonline/services/download_layer_resources/' + layer.workspace + '/' + layer.layer_name +'/';
			...
			ui += 	'</div>';
			ui += 	'<div class="col-md-4 form-group download-btn">';	
			ui += 		'<a target="_blank" href="' + layerResourcesLink + '" download="layer.resources"><div><i style="margin-right: 10px;" class="fa fa-download"></i>' + gettext('Download Layer Resources') + '</div></a>';
			ui += 	'</div>';
			ui += '</div>';
